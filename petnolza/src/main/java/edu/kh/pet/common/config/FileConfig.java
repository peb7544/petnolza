package edu.kh.pet.common.config;
...
public class FileConfig implements WebMvcConfigurer {
	...
	// 객실 예약 이미지
	@Value("${my.room.resource-handler}")
	private String roomResourceHandler;
	
	@Value("${my.room.resource-location}")
	private String roomResourceLocation;
	
	// 공지사항 파일
	@Value("${my.notice.resource-handler}")
	private String boardResourceHandler;
	
	@Value("${my.notice.resource-location}")
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		.addResourceLocations("file:///C:/petnolza/test/");
		// 클라이언트가 /myPage/file/** 패턴으로 이미지를 요청할 대
		// 요청을 연결해서 처리해줄 서버 폴더 경로 연결
		
		// 객실 예약 이미지 요청 - 서버 폴더 연결 추가
		registry
		.addResourceHandler(roomResourceHandler) // /myPage/profile/
		.addResourceLocations(roomResourceLocation); // file:///C:/uploadFiles/profile/
